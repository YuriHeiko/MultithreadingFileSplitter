...
class JoinChunk extends Chunk {
    JoinChunk(final IOHandler io, final StatisticHolder holder, final String fileToWriteName,
    ...
    public void run() {
        ...
        while (toDo > 0) {
            int done = doAction(chunkSize - toDo, pointer + chunkSize - toDo, buffSize);
